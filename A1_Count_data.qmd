...
tm_basemap()+
  ...
  tm_dots(fill = "black",alpha = 0.3,group = "Other sites")+
  tm_shape(sf_counts |>
             semi_join(no_flow_sites,
                       by = join_by(group_id,subgroup_id)))+
  tm_dots(fill = "red",group = "Zero counts sites")
```



Annual Average Daily Flow calculation
```{r}
...
tm_basemap()+
  ...
  tm_shape(sf_counts |>
             semi_join(high_change_counts,
                       by = join_by(group_id,subgroup_id)))+
  tm_dots(fill = "red",group = "High change sites")
```
