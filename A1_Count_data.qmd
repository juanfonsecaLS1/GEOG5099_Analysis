...
Two count points need to be adjusted manually: 
- there is no a valid description of the direction of `group_id` 136. See [this](https://maps.app.goo.gl/3rBHcqJooss6Z5Zp9).
- there is a typo in the direction of `group_id` 176

```{r}
sf_counts_selected$direction[sf_counts_selected$group_id == 136] <- "EASTBOUND"
sf_counts_selected$direction[sf_counts_selected$group_id == 176] <- "WESTBOUND"
```

```{r}
sf_counts_clean <- sf_counts_selected |> 
