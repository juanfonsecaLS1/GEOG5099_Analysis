...
#### Catchment of each link
Two main approaches will be used to estimate the extent to which each road provides access to the population and jobs in each LSOA. Firstly, it will be assumed that the length of each road will be proportional to the population served in each LSOA, i.e. if there are two roads in an LSOA, one with 750 m and the other with 250 m, their distribution will be assumed as 75% and 25% respectively. 
...
The second approach consist in producing a left-sided buffer for each link, the resulting areas will be used with the population and job density to estimate the number of inhabitants and jobs served. As an example
